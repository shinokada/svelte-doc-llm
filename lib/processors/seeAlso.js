...
export async function processSeeAlsoSection(content) {
  ...
  if (!relatedLinksMatch) {
    if (DEBUG) console.log('No relatedLinks found in script section');
    return content;
  }
  ...
  if (DEBUG) console.log('Found relatedLinks:', links);
  ...
  if (seeAlsoRegex.test(content)) {
    if (DEBUG) console.log('Replacing existing "See also" section');
    content = content.replace(seeAlsoRegex, relatedLinksMarkdown);
  } else {
    if (DEBUG) console.log('No "See also" section found, appending');
    content += `\n\n${relatedLinksMarkdown}`;
  }
