...
export async function processComponentDataSection(content, componentTitle, dataDir) {
  ...
  try {
    ...
    if (!componentsData.length) {
      if (DEBUG) console.warn(`No component data found for ${componentTitle}`);
      return content;
    }
    ...
    // Log for debugging
    if (DEBUG) console.log(`Replacing Component data section for ${componentTitle}`);
    if (DEBUG) console.log(`Old section length: ${componentDataSection.length}`);
    if (DEBUG) console.log(`New section length: ${newComponentDataSection.length}`);
    ...
    if (content === updatedContent) {
      ...
      if (DEBUG) console.log('Component data section regex match:');
      if (DEBUG) console.log('Match found:', !!componentDataMatch);
