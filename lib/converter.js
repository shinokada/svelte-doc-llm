...
const DEBUG = process.env.DEBUGCONVERTER === 'true';
/**
...
export async function generateContextFull (processedFiles, config) {
  try {
    ...
    if (DEBUG) console.log(`Starting context-full.txt generation with ${processedFiles.length} files`);
    if (DEBUG) console.log(`Output directory: ${outDir}`);

    // Group files by their directory (first part of relative path after 'docs')
    const filesByCategory = {};

    if (DEBUG) console.log('Processing files for context generation...');
    ...
    for (const file of processedFiles) {
      ...
      if (DEBUG) console.log(`File: ${file.relativePath} -> Category: ${category}`);
    ...
    if (DEBUG) console.log('Categories found:', Object.keys(filesByCategory));
    if (DEBUG) console.log('Files by category:', Object.fromEntries(
...
`;
    ...
    if (DEBUG) console.log(`Writing context-full.txt to: ${contextPath}`);
    await fs.writeFile(contextPath, contextContent);
    if (DEBUG) console.log(`Generated context-full.txt at ${contextPath}`);

    // Log statistics
    const totalFiles = processedFiles.length;
    const totalSize = Buffer.byteLength(contextContent, 'utf8');
    if (DEBUG) console.log(`Context file contains ${totalFiles} documents, ${Math.round(totalSize / 1024)} KB`);
...
async function processFile (filePath, outputDir, ignoreList, dataDir, format, stripPrefix, config) {
  try {
    ...
    if (DEBUG) console.log(`Output written to ${outputFilePath}`);
...
`;
  ...
  if (DEBUG) console.log(`Generated llms.txt at ${llmsTxtPath}`);
}
...
async function processMarkdown (content, componentTitle, ignoreList, dataDir, currentFilePath, config) {
  ...
  if (dependencies.length > 0 && DEBUG) {
