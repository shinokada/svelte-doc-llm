...
`;
    ...
    if (failedFiles.length > 0) {
      console.warn(
        `⚠️  Warning: ${failedFiles.length} file(s) could not be included in context-full.txt:`
      );
    ...
    if (DEBUG) {
      console.log(
        `Context file contains ${totalFiles} documents, ${Math.round(totalSize / 1024)} KB`
      );
    }
  } catch (error) {
...
async function processFile(filePath, outputDir, ignoreList, dataDir, format, stripPrefix, config) {
  try {
    ...
    processedContent += await processMarkdown(
      markdownContent,
      component_title,
      ignoreList,
      dataDir,
    ...
    const relativePath = path.relative(
      path.resolve(process.cwd(), config.srcDir || 'src/routes'),
      filePath
    );

    // Apply stripPrefix if configured
    const outputRelativePath = applyStripPrefix(relativePath, stripPrefix);
...
`;
  ...
  for (const file of sortedFiles) {
    const urlPath = file.relativePath.replace(/\\/g, '/').replace(/\.[^.]+$/, `.${fmt}`);
...
async function processMarkdown(
  ...
  const { docsDir = 'src/routes', examplesDir = 'src/routes/docs-examples' } = config;
